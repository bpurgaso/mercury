...
pub fn start_sfu(config: &MediaConfig) -> (SfuHandle, mpsc::Receiver<SfuEvent>) {
    ...
    // Spawn the command loop on the dedicated runtime
    rt.spawn(sfu_command_loop(
        command_rx,
        event_tx,
        max_participants,
        empty_room_timeout,
...
/// Main command processing loop for the SFU.
async fn sfu_command_loop(
    mut command_rx: mpsc::Receiver<SfuCommand>,
    event_tx: mpsc::Sender<SfuEvent>,
    max_participants: usize,
    empty_room_timeout: Duration,
) {
    let mut room_manager = RoomManager::new(max_participants, empty_room_timeout, event_tx);
    ...
    while let Some(cmd) = command_rx.recv().await {
        match cmd {
            ...
            SfuCommand::WebRtcSignal {
                ...
                let result = room_manager.handle_signal(user_id, &room_id, signal).await;
                let _ = reply.send(result);
            }
